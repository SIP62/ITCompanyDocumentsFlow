...
public class UserExcelOutput {
	...
	public boolean sendDataToExcel(List<Users> userList) throws IOException {
	    ...
	    styleTd.setBorderTop(XSSFCellStyle.BORDER_THIN);
	      ...
	      if (!userList.isEmpty()) {
	    	  for (Users user: userList) {
			      ...
			      cell.setCellStyle(styleTd);
			      cell=row.createCell(2);
			      cell.setCellValue(user.getRoles().getRole());
			      cell.setCellStyle(styleTd);
			      i++;
		      }
	      }
	      ...
	      FileOutputStream out = new FileOutputStream(
	    	      new File("userlistoutput.xlsx"));
	      workbook.write(out);
	      out.close();
	      isOutput = true;
	      System.out.println(
	    	      "userlistoutput.xlsx written successfully");
		return isOutput;
	}
